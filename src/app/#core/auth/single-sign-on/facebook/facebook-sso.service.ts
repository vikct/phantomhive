...
export class FacebookSsoService {
  ...
  async login(): Promise<any> {
    try {
      ...
      if (err.code === 'auth/account-exists-with-different-credential') {
        console.error(
          'Facebook login failed: An account already exists with the same email but a different login provider.'
        );
  ...
  async logout(): Promise<void> {
    try {
      ...
      console.log('Signed out from Firebase via FacebookService');
    } catch (err) {
